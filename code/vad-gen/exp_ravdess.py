...
POSTER_MATCH = {
    'Neutral':'Neutral','Happiness':'Happiness','Sadness':'Sadness',
    'Anger':'Anger','Fear':'Fear','Disgust':'Disgust','Surprise':'Surprise'
}
...
def crop_face(frame, det, margin=0.2):
    ...
    x=max(0,int((bb.xmin-bb.width*margin/2)*w)); y=max(0,int((bb.ymin-bb.height*margin/2)*h))
    x2=min(w,int((bb.xmin+bb.width*(1+margin/2))*w)); y2=min(h,int((bb.ymin+bb.height*(1+margin/2))*h))
...
def main():
    ...
    for gt in sorted(confusion.keys()):
        ...
        preds = ', '.join(f"{p}:{c}" for p,c in sorted(confusion[gt].items(), key=lambda x:-x[1]))
