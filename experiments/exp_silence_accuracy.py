...
def run_file(target, npz_files, sample_every=5):
    npz = next((path for path in npz_files if target in path), None)
    if npz is None:
        raise FileNotFoundError(f"Seamless 입력을 찾을 수 없습니다: {target}")
    ...
    print(
        f"[{target}] 발화: {speech_correct}/{speech_total} ({emo_acc_speech:.1f}%), "
...
def main():
    ...
    print("\n" + "=" * 50)
    print("=== 전체 종합 ===")
    speech_accuracy = 100 * tot_sc / tot_st if tot_st else 0.0
    silence_accuracy = 100 * tot_zc / tot_zt if tot_zt else 0.0
    print(f"발화 구간 정확도: {tot_sc}/{tot_st} ({speech_accuracy:.1f}%)")
    print(f"침묵 구간 정확도: {tot_zc}/{tot_zt} ({silence_accuracy:.1f}%)")
    print("=" * 50)
