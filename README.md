...
2026-09-04 수집. MIPA 초기 구현과 vad-gen 후속 구현의 코드·기존 산출물·연구 계획을 모았습니다. **논문 최종 초안과 정량 평가 결과 전체가 확보된 상태는 아니며, 교수님 제출용 설치 검증도 아직 완료되지 않았습니다.**
...
Python 3에서 묶음 폴더를 기준으로 실행합니다. 외부 패키지·모델·GPU가 필요 없는 합성 타임라인 → SRT 확인입니다.

```bash
python3 tools/smoke_test.py
```

성공하면 `PASS`와 합성 SRT가 출력됩니다. 이는 표정·음성 모델이나 LLM 품질 검증이 아닙니다.
...
- 연구일지에 기록된 8월 논문 초안·수정본과 교수님 피드백 원본
- RAVDESS 및 발화/침묵 비교 실험의 최종 수치·실행 로그·원시 예측
- 사람 평가표, 정답 주석, 비교군 결과, 최종 그림·표 원본
- GitHub 저장소 주소와 외부 사용자가 받을 수 있는 데이터·가중치 경로의 검증

AI 대화와 코드 주석의 성능·신규성 주장은 검증된 실험 결과로 취급하지 않습니다.
