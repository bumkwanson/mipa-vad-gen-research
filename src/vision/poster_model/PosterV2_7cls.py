...
class pyramid_trans_expr2(nn.Module):
    def __init__(self, img_size=224, num_classes=7, window_size=[28,14,7], num_heads=[2, 4, 8], dims=[64, 128, 256], embed_dim=768,
                 mobileface_checkpoint_path=None, ir_checkpoint_path=None):
        super().__init__()

        project_root = Path(__file__).resolve().parents[3]
        if mobileface_checkpoint_path is None:
            mobileface_checkpoint_path = project_root / 'weights' / 'pretrain' / 'mobilefacenet_model_best.pth.tar'
        if ir_checkpoint_path is None:
            ir_checkpoint_path = project_root / 'weights' / 'pretrain' / 'ir50.pth'
        ...
        face_landback_checkpoint = torch.load(mobileface_checkpoint_path, map_location='cpu')
        ...
        ir_checkpoint = torch.load(ir_checkpoint_path, map_location='cpu')
